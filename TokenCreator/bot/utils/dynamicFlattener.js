/**
 * Dynamic Contract Flattener
 * Generates flattened contract code based on token parameters
 * Ensures code matches deployment exactly
 */
...
/**
 * Generate flattened SecureToken with exact parameters
 * This ensures the generated code matches what was deployed
 */
function generateFlattenedSecureToken(tokenParams = {}) {
    const {
        name = "DefaultToken",
        symbol = "DFT",
        taxPercent = 0,
    ...
    // This template is EXACTLY what compiles to the bytecode deployed
    // Any changes here will cause verification to fail
...
/**
 * Get verification data for a deployed token
 */
function getVerificationData(deploymentReceipt, tokenParams) {
    // Convert BigInt values to strings for JSON serialization
    const initialSupply = typeof tokenParams.initialSupply === 'bigint' 
        ? tokenParams.initialSupply.toString() 
        : tokenParams.initialSupply;
    
    const blockNumber = typeof deploymentReceipt.blockNumber === 'bigint'
        ? deploymentReceipt.blockNumber.toString()
        : deploymentReceipt.blockNumber;
    
    const gasUsed = deploymentReceipt.gasUsed 
        ? (typeof deploymentReceipt.gasUsed === 'bigint' 
            ? deploymentReceipt.gasUsed.toString() 
            : (typeof deploymentReceipt.gasUsed.toString === 'function' 
                ? deploymentReceipt.gasUsed.toString() 
                : String(deploymentReceipt.gasUsed)))
        : '0';
    ...
    return {
        ...
        constructorArguments: [
            tokenParams.name,
            tokenParams.symbol,
            initialSupply, // Now a string if it was BigInt
            tokenParams.taxPercent || 0,
            tokenParams.taxWallet || "0x0000000000000000000000000000000000000000",
            tokenParams.reflectionPercent || 0,
            tokenParams.burnPercent || 0,
            tokenParams.enableReflection || false,
            tokenParams.enableBurn || false,
            tokenParams.owner
        ],
        flattenedCode: generateFlattenedSecureToken(tokenParams),
        verificationUrl: `https://alveyscan.com/address/${deploymentReceipt.contractAddress}#code`
    };
}

module.exports = {
    generateFlattenedSecureToken,
    getVerificationData
};
